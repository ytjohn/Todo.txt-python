...
class TodoDotTxt():
    ...
    def iter_todos(self, include_done=False):
        """Opens the file in read-only mode; returns an iterator for the
        todos."""
    ...
    def separate_line(self, number):
        ...
        the item at that position in the list. The list is the rest of the
        todos.

        If the todo.txt file is empty, separate = lines = None. If the
        number is invalid separate = None, lines != None."""
    ...
    def rewrite_file(self, fd, lines):
        """Simple wrapper for three lines used all too frequently. Sets the
        access position to the beginning of the file,
        tuncates the file's length to 0 and then writes all the lines to
        the file."""
        fd.seek(0, 0)
        fd.truncate(0)
        fd.writelines(lines)
    ...
    def _git_status(self):
        """Print the status of the local repository if the version of git is
         1.7 or later."""
    ...
    def _git_commit(self, files, message):
        ...
        try:
            ...
            self._git_err(g)
        committed = \
            self.config["TODO_DIR"] if "-a" in files else self.concat(files,
                                                                      ", ")
    ...
    def _iter_actual_lines_(self, config_file):
        """Return only the actual lines of the config file. This skips
    ...
    def get_config(self, config_name="", dir_name=""):
        ...
        if dir_name:
            ...
            self.config["TODOTXT_CFG_FILE"] = self._pathc([dir_name,
                                                           "/config"])
    ...
    def git_functions(self):
        ...
        def repo_config(self):
            ...
            if yes_re.match(ret):
                ...
                remote_host = \
                    __while_prompt__("Remote hostname:",
                                     "Please enter the remote's hostname.")
                remote_path = \
                    __while_prompt__("Remote path:",
                                     "Please enter the path to the remote's "
                                     "repository.")
                remote_user = \
                    __while_prompt__("Remote user:",
                                     "Please enter the user on the remote "
                                     "machine.")
                remote_branch = \
                    __while_prompt__("Remote branch:",
                                     "Please enter the branch to push to on "
                                     "the remote machine.")

                self.prompt("Press enter when you have initialized a bare\n",
                            " repository on the remote or are ready to "
                            "proceed.")
                ...
                g.remote("add",
                         "origin",
                         self.concat([remote_user, "@", remote_host, ":",
                                      remote_path]))
                g.config(self.concat(["branch.", local_branch, ".remote"]),
                         "origin")
    ...
    def default_config(self):
        ...
        if self.config["USE_GIT"]:
            ...
            try:
                ...
                val = self.prompt("Would you like to create a new git "
                                  "repository in\n ",
                                  self.config["TODO_DIR"], "? [y/N]")
                if yes_re.match(val):
                    print(self.config["GIT"].init())
                    val = self.prompt("Would you like {prog} to help\n you",
                                      " configure your new git repository? "
                                      "[y/n]", prog=self.config["TODO_PY"])

                    if yes_re.match(val):
                        repo_config()
                        files = [self.config["TODOTXT_CFG_FILE"],
                                 self.config["TODO_FILE"]]
                        for setting in ["TMP_FILE",
                                        "DONE_FILE",
                                        "REPORT_FILE"]:
                            if self.config[setting]:
                                files.append(self.config[setting])
                        self.config["GIT"].add(files)
                        self.config["GIT"].commit("-m",
                                                  '"%s initial commit."' %
                                                  self.config["TODO_PY"])
#                            self.concat(['"', self.config["TODO_PY"],
#                                         " initial commit.\""]))
                else:
                    ...
                    if yes_re.match(val):
                        ...
                        val = self.prompt("Please enter "
                                          "user@remote:/path/to/repo.")
                        git.Repo.clone_from(val, self.config["TODO_DIR"])

        cfg.close()

        print "Default configuration completed. Please re-run %s with '-h' " \
              "and 'help' separately." % self.config["TODO_PY"]
...
#                self._git_commit([self.config["TODO_FILE"]], removed)
    ...
    def format_lines(self, color_only=False, include_done=False):
        ...
        colors = set(self.term_colors.keys())
        # Supposedly sets are faster for look-ups
    ...
    def _list_by_(self, *args):
        ...
        for a in args:
            relist.append(
                re.compile(self.concat(["\s?(", esc(a), ")\s?"]), re.I))
    ...
    @usage('listall|lsa',
           ['Lists all items in your todo.txt file sorted by priority '
            'followed', 'by the items in your done.txt file.'])
    def list_all(self):
    ...
    def toggle_opt(self, option, opt_str, val, parser):
        """
        Check opt_str to see if it's one of ['-+', '-@', '-#', '-p', '-P',
        '-t', '--plain-mode', '--no-priority', '--prepend-date', '-i',
    ...
    def execute_commands(self, args):
        ...
        actions_dir = self.config.get('TODO_ACTIONS_DIR',
                                      self._pathc([self.config['TODO_DIR'],
                                                  '/actions']))
